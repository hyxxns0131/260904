# 설문조사 앱
# 전송버튼, 체크박스, 라디오 단추, 셀렉트 박스, 멀티 셀렉트 박스, 슬라이더, 텍스트입력
# 위젯
...
# age = st.text_input("1) 나이를입력하세요", value="50", key="widget_name")
...
# => score = st.slider("7) 이 강의 만족도 점수(1~5점)",1,5,5, key="widget_score")

# 8) 텍스트 영역 : 여러줄의 입력이 필요할 때 자유 의견 입력
...
if submitted :
    ...
    st.write(f"- 관심분야 : **{','.join(interests) if interests else '선택 없음'}**")
